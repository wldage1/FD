...
function showTypeChoose(showType,isRequired,id,checkValue,prdValue,rowCount){
	...
	$(document).ready(function() {
		...
		// 切换产品属性
		changeType($("#investCategoryId option:first").val());
		// 切换产品销售状态
	...
	function ajaxSave(){
		...
		var form = $("#product_"+currTab).serializeArray();
		
		var attributeStatus = true;
		if(currTab == 3){
			attributeStatus = attributeValid();
		}
		if(attributeStatus){
			var layers =  top.layer.load(0,2);
			jQuery.ajax({
				...
				success:function(data){
					if(data.status == 'success'){
						...
						$("#c").val("${product_manage_modify.code}");
						$("#form_1_id").val(productId);
						$("#form_2_id").val(productId);
						$("#form_3_id").val(productId);
						$("#form_4_id").val(productId);
						$("#form_2_fileName").val(fileName);
						$("#form_3_fileName").val(fileName);
						$("#form_4_fileName").val(fileName);
						tab(currTab);
						isSaved = true;
						top.layer.close(layers);
						$.layer({
							area: ['300px','auto'],
							border : [0, 0.5, '#666', true],
							dialog : {msg:'产品保存成功',type : 9}	
						});
					}else{
						top.layer.msg("产品保存失败");
					}
				}
			});
		}
...
<body class="skinMain">
	...
	<table width="100%" border="0" cellspacing="1" cellpadding="0" class="skinMain">
		<tr>
			<td width="100%">
			<div id="div_1" >
			<form:form method="post" action="save" commandName="product" id="product_1" >
				<input type="hidden" name="c" id="c" value="${c}"/>
				<input type="hidden" name="id" id="form_1_id"/>
				<input type="hidden" name="prompt" value="name"/>
				<input type="hidden" name="turnCode" value="${product_manage.code}"/>
				<input type="hidden" name="coverParam" value="0"/>
				<input type="hidden" name="productImage" id="productImage" value="" />
				<input type="hidden" name="fileName" id="fileName" value="" />
				<input type="hidden" name="orgTypeId" id="orgTypeId" value="">
				<c:if test="${isCommission == 1}">
				<input type="hidden" name="orgId" value="${selfOrgId}">
				</c:if>
			<table width="100%" border="0" cellspacing="1" cellpadding="0" class="skinMain">
				<tr>
					<td width="100%">
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="skinLayout">
							...
							<tr>
								...
								<td width="85%" class="skinLayout_lable">
									<form:select path="type" class="valid-control" htmlEscape="true" onChange="changeProductType($(this).val());">
										<option value="">请选择</option>
							...
							<tr>
								...
								<td width="85%" class="skinLayout_lable">
									<form:select path="raiseArea">
										<option value="">请选择</option>
										<c:forEach items="${raiseArea}" var="item">
											<option value="${item.key}">${item.value}</option>
										</c:forEach>										
									</form:select><font color="red">&nbsp;&nbsp;</font>
								</td>
							...
							<tr>
								...
								<td width="85%" class="skinLayout_lable">
									<input type="text" id="raiseStartTime" name="raiseStartTime" onfocus="WdatePicker({maxDate:'#F{$dp.$D(\'raiseFinishTime\')||\'2050-01-01\'}'})" class="Wdate valid-control">&nbsp;<font color="red" class="tip-time">*</font>
								</td>
							</tr>
							<tr>
								...
								<td width="85%" class="skinLayout_lable">
									<input type="text" id="raiseFinishTime" name="raiseFinishTime" value="" onfocus="WdatePicker({minDate:'#F{$dp.$D(\'raiseStartTime\')}',maxDate:'2050-01-01'})" class="Wdate valid-control">&nbsp;<font color="red" class="tip-time">*</font>
								</td>
							</tr>
							<tr>
	...
	<div id="div_2" style="display:none;">
		<form:form method="post" action="save" commandName="product" id="product_2" >
		<input type="hidden" name="c" value="${product_manage_modify.code}"/>
		<input type="hidden" name="id" id="form_2_id"/>
		<input type="hidden" name="fileName" id="form_2_fileName"/>
	...
	<div id="div_3" style="display:none;">
		<form:form method="post" action="save" commandName="product" id="product_3" >
		<input type="hidden" name="c" value="${product_manage_modify.code}"/>
		<input type="hidden" name="id" id="form_3_id"/>
		<input type="hidden" name="fileName" id="form_3_fileName"/>
		<table  width="100%" border="0" cellspacing="1" cellpadding="0" class="skinMain">
			<tr>
	...
	<div id="div_4" style="display:none;">
		<form:form method="post" action="save" commandName="product" id="product_4" >
		<input type="hidden" name="c" value="${product_manage_modify.code}"/>
		<input type="hidden" name="id" id="form_4_id"/>
		<input type="hidden" name="fileName" id="form_4_fileName"/>
		<table width="100%" border="0" cellspacing="0" cellpadding="0" >
