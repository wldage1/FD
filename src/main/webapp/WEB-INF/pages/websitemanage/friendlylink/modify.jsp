...
$(document).ready(function(){
    var options = {
			event:'focus',
			ajax:{
				url:'${base}/websitemanage/friendlylink/valid.json'
			},
	...
	$("#uploadify").uploadify({
   	...
   	//后台处理的请求的servlet,同时传递参数,由于servlet只能接收一个参数，所以将两个参数合并成一个。   
      	'uploader'       : '${base}/websitemanage/friendlylink/uploadfile;jsessionid=<%=session.getId()%>',
       'buttonClass'    : 'btn2',
       'queueID'        : 'fileQueue',
	...
	//	'uploadLimit'    : 1,    //用于同时上传的数目
		'onInit'      	 : function() {
			...
			if(logoUrl == "" || logoUrl == null){
				...
				$("#fileDel").html("<img height='100' src='${ftpUrl}${links.logo}'>&nbsp;&nbsp;&nbsp;&nbsp;<a href='#' class='delMongoFile'><msg:message code='button.delete'/></a>");
			}
		},
...
<table cellpadding="0" cellspacing="0" border="0" width="100%">
	<tr>
		<td>
			<form:form method="post" action="save" commandName="links">
				<input type="hidden" name="c" value="${links.c}"/>
				<input type="hidden" name="id" value="${links.id}"/>
				<input type="hidden" name="prompt" value="name" />
				<input type="hidden" name="turnCode" value="${websitemanage_friendlylink.code}" />
				<input type="hidden" name="coverParam" value="1" />
				<input type="hidden" name="logo" id="logo" value="${links.logo}" />
				<input type="hidden" name="picName" id="picName" />
				<table width="100%" border="0" cellspacing="0" cellpadding="0" class="skinLayout">
					<tr>
						<td width="15%" align="right" class="skinLayout_info"><msg:message code="links.name"/><msg:message code="system.common.sign.colon"/></td>
						<td width="85%" class="skinLayout_lable">
							<input name="name" type="text" id="name" style="width: 200;" value="${links.name}"  class="valid-control"/><font color="red">&nbsp;*&nbsp;</font>
						</td>
					</tr>
					<tr>
						<td width="15%" align="right" class="skinLayout_info"><msg:message code="links.url"/><msg:message code="system.common.sign.colon"/></td>
						<td width="85%" class="skinLayout_lable">
							<form:input path="url" id="url" maxlength="200" value="${links.url}" class="valid-control"/><font color="red">&nbsp;*&nbsp;</font>
						</td>
					</tr>
					<tr>
						<td width="15%" align="right" class="skinLayout_info"><msg:message code="links.logo"/><msg:message code="system.common.sign.colon"/></td>
						<td width="85%" class="skinLayout_lable">
