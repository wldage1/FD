...
<script type="text/javascript">
	...
	$(document).ready(function(){
		jQuery("#unfeeList").jqGrid({
			...
			datatype: "json",
			colNames:['','','团队简称','累计返利成交规模（万）','成功系数','奖励分值（万）','返利分值（万）','居间费总额（元）','返利比例','返利金额（元）','操作'],
		...
		if(payStatus==0){
			...
			var layers = top.layer.confirm("<msg:message code='incentive.ispay.comfirm' />",  function(){
				$.ajax({
					...
					timeout:'60000',
					dataType:'json',
					data:{c:'${incentivefee_organizationfeemanage_onekeypay.code}',orgIds:sourceIds,incentiveFeeIds:incentiveFeeIds,auditingTime:auditingTime,companyID:$("#companyID").val(),type:type},
					success:function(jsonData,textStatus){
						if(textStatus == "success"){
							if(jsonData.status == "success"){
								top.layer.close(layers);
