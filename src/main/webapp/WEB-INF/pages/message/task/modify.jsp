...
$(document).ready(function(){
	//初始化发送类型
	var sendTypeValue = $(":radio[name='sendType']:checked").val();
	if(sendTypeValue != 2){
		$("#tr_frequency").hide();
...
function initSendFrequency(){
	var sendFrequency = '${messageTask.sendFrequency}';
	var quartzMonth = '${messageTask.quartzMonth}';
	var quartzWeek = '${messageTask.quartzWeek}';
	var quartzDay = '${messageTask.quartzDay}';
	var quartzHour = '${messageTask.quartzHour}';
	switch (parseInt(sendFrequency)){
	case 1:
		var timeSelect = packageSelect("quartzHour", 24,quartzHour);
		content = "每天" + timeSelect + "点发送消息。";
		break;
	case 2:
		var timeSelect = packageSelect("quartzHour", 24,quartzHour);
		var weekSelect = packageSelect("quartzWeek", 7,quartzWeek);
		content = "每周" + weekSelect + "," + timeSelect + "点发送消息。";
		break;
	case 3:
		var timeSelect = packageSelect("quartzHour", 24,quartzHour);
		var daySelect = packageSelect("quartzDay", 31,quartzDay);
		content = "每月" + daySelect + "号" + timeSelect + "点发送消息。";
		break;
	case 4:
		var timeSelect = packageSelect("quartzHour", 24,quartzHour);
		var daySelect = packageSelect("quartzDay", 31,quartzDay);
		var monthSelect = packageSelect("quartzMonth", 12,quartzMonth);
...
function packageSelect(selectName, endNumber,selectValue){
	var content;
...
input[type="checkbox"] {
	...
	<form:form method="post" action="save"  commandName="messageTask" >
