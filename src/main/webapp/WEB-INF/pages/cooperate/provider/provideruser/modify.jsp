...
$(document).ready(function(){
	...
	var auths = $("input:checkbox[name=authorization]");
	for(var i=0;i < auths.length;i++){
		for(var j=0;j<auth.length;j++){
			if(auths[i].value == auth[j]){
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" class="skinMain">
	...
	<tr>
		<td width="100%">
			...
			<input type="hidden" name="id" value="${providerUser.id}" />
			<input type="hidden" name="prompt" value="name" />
			<input type="hidden" name="c" value="${providerUser.c}"/>
			<input type="hidden" name="account" value="${providerUser.account}"/>
			<input type="hidden" name="operate" value="2" />
				<table width="100%" border="0" cellspacing="0" cellpadding="0" class="skinLayout">
					...
					<tr>
						...
						<td width="70%" class="skinLayout_lable">
							<form:textarea path="description" maxlength="200" rows="3" cols="28" onkeypress="return imposeMaxLength(this, 100)" cssStyle="width: 250px; height: 85px;"/><font color="red">&nbsp;&nbsp;<form:errors path="description" /></font>
						</td>
					</tr>
				</table>
