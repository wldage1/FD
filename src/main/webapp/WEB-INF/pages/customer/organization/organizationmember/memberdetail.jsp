...
<html>
	<head>
		...
		$(document).ready(function(){
			<c:if test="${!empty customer_organization_account}">
		    <security:authorize ifAnyGranted="${customer_organization_account.code}">
			$("#accountDetail").click(function(){
				$.ajax({
   					url: '${base}${customer_organization_account.controller}.json',
   					type:'post',
   					timeout:'60000',
   					dataType:'json',
   					data: {id:${member.id}, c:'${customer_organization_account.code}'}, 
   					async:false,
   					cache:false,
		    ...
		    <c:if test="${!empty customer_organization_idcard}">
		    <security:authorize ifAnyGranted="${customer_organization_idcard.code}">
			$("#idCardDetail").click(function(){
				$.ajax({
   					url: '${base}${customer_organization_idcard.controller}.json',
   					type:'post',
   					timeout:'60000',
   					dataType:'json',
   					data: {id:${member.id}, c:'${customer_organization_idcard.code}'}, 
   					async:false,
   					cache:false,
   					success:function(jsonData){
   						if(jsonData.idcard){
			...
			<c:if test="${!empty customer_organization_mobilephone}">
		    <security:authorize ifAnyGranted="${customer_organization_mobilephone.code}">
			$("#mobilePhoneDetail").click(function(){
				$.ajax({
   					url: '${base}${customer_organization_mobilephone.controller}.json',
   					type:'post',
   					timeout:'60000',
   					dataType:'json',
   					data: {id:${member.id}, c:'${customer_organization_mobilephone.code}'}, 
   					async:false,
   					cache:false,
   					success:function(jsonData){
		    ...
		    <c:if test="${!empty customer_organization_homephone}">
		    <security:authorize ifAnyGranted="${customer_organization_homephone.code}">
			$("#homePhoneDetail").click(function(){
				$.ajax({
   					url: '${base}${customer_organization_homephone.controller}.json',
   					type:'post',
   					timeout:'60000',
   					dataType:'json',
   					data: {id:${member.id}, c:'${customer_organization_homephone.code}'}, 
   					async:false,
   					cache:false,
   					success:function(jsonData){
	...
	<body class="skinMain">
	<input type="hidden" name="turnCode" value="${customer_organization_organizationmember.code}"/>
