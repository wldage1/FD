...
$(document).ready(function(){
    jQuery("#memberList").jqGrid({
        ...
        url: '${base}/customer/organizationmember/grid.json',
        datatype: 'json',
        colNames: [
        ...
        colModel: [
                   ...
                   {name:'act',index:'act', width:'30%',align:'center',sortable:false,
                	  formatter:function(cellvalue, options, rowdata){
				    			...
				    			var space = "&nbsp;&nbsp;";
				    			var content = "";
				    		    <c:if test="${!empty customer_organization_organizationmember_memberdetail}">
				    		    <security:authorize ifAnyGranted="${customer_organization_organizationmember_memberdetail.code}">
				   		    	content += "<a href='javascript:void(0);' title='${customer_organization_organizationmember_memberdetail.name}' id='" + id + "' class='shortcut_${customer_organization_organizationmember_memberdetail.indexCode}'>";
				   		    	content += "<img src='${base}/${skin}/images/icon/detail.png' weight='18' height='18' border='0' align='absmiddle'/>${customer_organization_organizationmember_memberdetail.name}";
				   		    	content += "</a>";
				    		    </security:authorize>
				    		    </c:if>  
				    		    return content;
    ...
    //理财师详细的跳转
     $(".shortcut_${customer_organization_organizationmember_memberdetail.indexCode}").live("click",function(){
	   	var rowid = $(this).attr("id");
		window.location.href = "${base}${customer_organization_organizationmember_memberdetail.controller}?c=${customer_organization_organizationmember_memberdetail.code}&id="+ rowid;
   	 });
